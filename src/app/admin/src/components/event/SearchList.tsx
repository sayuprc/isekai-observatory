...
const statusName: Record<number, string> = { 1: '延期', 2: '中止' };
...
export const SearchList = () => {
  ...
  const [data, { refetch }] = createResource(() => ({ title: title(), type: type(), status: status(), order: 'asc' as SortOrder }), async (params) => {
    setFetchError(null);
    const response = await client.api.events.search.get({ query: { title: params.title || undefined, type: params.type ? Number(params.type) as EventTypeValue : undefined, status: params.status ? Number(params.status) as EventStatusValue : undefined, sort: 'schedule' as EventSearchSortBy, order: params.order, page: 1, per_page: 100 } });
    if (!response.data) {
  ...
  return (
    <>
      ...
      >
        ...
        <fieldset class="fieldset"><label class="fieldset-label" for="event-status">状態</label><select id="event-status" class="select select-bordered select-sm" value={status()} onChange={e => setStatus(e.currentTarget.value)}><option value="">すべて</option><option value="1">延期</option><option value="2">中止</option></select></fieldset>
